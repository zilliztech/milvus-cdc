...
def hello_milvus(host="127.0.0.1"):
    ...
    print(f"\nCreate collection...")
    collection = Collection(name="insert_demo", schema=default_schema)
    ...
    for i in range(1000):
        ...
        collection.insert(
            [
                [i for i in range(batch*i, batch*(i+1))],
                [np.float32(i) for i in range(batch*i, batch*(i+1))],
                [str(i) for i in range(batch*i, batch*(i+1))],
                vectors
            ]
        )
        t1 = time.time()
        print(f"\nInsert {batch} vectors cost {t1 - t0:.4f} seconds")
        print(f"id {[i for i in range(batch*i, batch*(i+1))]}")
        time.sleep(10)
